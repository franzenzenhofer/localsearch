...
async function deploy() {
  ...
  try {
    ...
    exec(
      "wrangler pages deploy dist --project-name=filesearch --commit-dirty=true",
    );
    ...
    try {
      ...
      log.warning("Cache purge failed (expected with current API permissions)");
...
Co-Authored-By: Claude <noreply@anthropic.com>"`);
    ...
    if (!tagExists) {
      exec(
        `git tag -a "v${version}" -m "LocalSearch v${version} - Professional File Search"`,
      );
