...
const SITE_URL = process.argv[2] || "https://filesearch.franzai.com";
...
class PostDeploymentTester {
  ...
  async testLiveVersionVerification() {
    ...
    const titleMatch = html.match(
      /<title>FileSearch v([^<]+) - Private File Search<\/title>/,
    );
    if (!titleMatch) {
      return `❌ CRITICAL: Version not found in HTML title. Expected "FileSearch v${expectedVersion}" but title is: ${html.match(/<title>([^<]+)<\/title>/)?.[1] || "MISSING"}`;
    }
    ...
    if (
      !html.includes(`<meta name="app-version" content="${expectedVersion}"`)
    ) {
    ...
    success(
      `🎉 CONFIRMED: Live site shows correct version v${expectedVersion}`,
    );
    success(`🔍 Title: FileSearch v${titleVersion} ✅`);
  ...
  async testPWAManifest() {
    ...
    if (!manifest.name || manifest.name !== "FileSearch") {
      return "PWA manifest missing or incorrect name";
    }

    if (!manifest.theme_color || manifest.theme_color !== "#FFD700") {
  ...
  async testMobileViewport() {
    ...
    if (!html.includes("theme-color") || !html.includes("#FFD700")) {
  ...
  async testBrandingElements() {
    ...
    const hasVersionedTitle =
      /<title>FileSearch v[0-9]+\.[0-9]+\.[0-9]+ - Private File Search<\/title>/.test(
        html,
      );
  ...
  async runAllTests() {
    ...
    await this.runTest("🔍 LIVE VERSION VERIFICATION", () =>
      this.testLiveVersionVerification(),
    );

    // Core functionality tests
    await this.runTest("Site Accessibility", () =>
      this.testSiteAccessibility(),
    );
    await this.runTest("Static Assets Loading", () => this.testStaticAssets());
    await this.runTest("Material-UI Integration", () =>
      this.testMaterialUILoading(),
    );
    ...
    fs.writeFileSync(
      "/tmp/localsearch-post-deploy-test-report.json",
      JSON.stringify(report, null, 2),
    );
    info(
      "📄 Test report saved to /tmp/localsearch-post-deploy-test-report.json",
    );
