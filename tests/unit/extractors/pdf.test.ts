...
describe("PDFExtractor", () => {
  ...
  describe("extractText", () => {
    ...
    it("should handle multi-page PDFs", async () => {
      ...
      const mockPage2 = {
        getTextContent: vi.fn().mockResolvedValue({
          items: [{ str: "Page 2 content", transform: [1, 0, 0, 1, 0, 0] }],
        }),
      };
    ...
    it("should handle PDF parsing errors gracefully", async () => {
      const pdfBuffer = new ArrayBuffer(1024);
      mockPdfDocument.getPage.mockRejectedValue(
        new Error("PDF parsing failed"),
      );

      await expect(
        extractor.extractText(pdfBuffer, sampleMetadata),
  ...
  describe("extract", () => {
    it("should return DocumentContent with extracted text", async () => {
      ...
      const extractTextSpy = vi
        .spyOn(extractor, "extractText")
        .mockResolvedValue("Hello world! This is test content.");
