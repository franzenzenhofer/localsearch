...
describe("Code Quality - Line Count Enforcement", () => {
  ...
  it("should enforce 75-line limit on ALL files", () => {
    ...
    if (violations.length > 0) {
      ...
      violations.forEach(({ file, lines }) => {
        console.error(
          `  ${file}: ${lines} lines (${lines - maxLines} over limit)`,
        );
      });
  ...
  it("should encourage even smaller files", () => {
    const idealMax = 50;
    const largeFiles = Array.from(fileLineCounts.entries()).filter(
      ([, lines]) => lines > idealMax,
    ).length;
    ...
    console.log(
      `  Files under 50 lines: ${totalFiles - largeFiles}/${totalFiles} (${smallFileRatio.toFixed(1)}%)`,
    );
    /* eslint-enable no-console */
  ...
  it("should track average file size", () => {
    ...
    const averageLines =
      allLineCounts.reduce((sum, lines) => sum + lines, 0) /
      allLineCounts.length;
