...
describe("DexieStorage - Basic Operations", () => {
  ...
  it("should initialize database correctly", async () => {
    const { DexieStorage } = await import(
      "../../../src/storage/dexie-storage.js"
    );
  ...
  it("should add documents successfully", async () => {
    ...
    const documents = [{ id: "1", fileId: "file1", text: "test content" }];

    expect(storage).toBeDefined();
  });

  it("should search documents successfully", async () => {
    const mockResults = [{ id: "1", fileId: "file1", text: "test content" }];
