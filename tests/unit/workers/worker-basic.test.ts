...
describe("IndexingWorkerClass - Basic Operations", () => {
  it("should extract text from TXT files", async () => {
    const { IndexingWorkerClass } = await import(
      "../../../src/workers/indexing-worker.js"
    );
  ...
  it("should handle unsupported file types", async () => {
    ...
    const file = new File(["unknown content"], "test.unknown", {
    ...
    await expect(worker.extractText(file, metadata)).rejects.toThrow(
